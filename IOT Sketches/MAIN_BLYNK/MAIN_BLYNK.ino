...
void loop(){
  ...
  if(taskBlynkWrite.check()){
   blynkTransmit(temp, humidity, pressure, CO2, CO, NH4, uvIndex);
  }
...
void blynkTransmit(float temp, float humidity, float pressure, float CO2, float CO, float NH4, float UVI){
  int uvIndex = UVI;
